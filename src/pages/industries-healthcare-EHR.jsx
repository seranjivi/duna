          ...
          <div className="grid gap-12 lg:grid-cols-[1fr_1fr] lg:items-center">
            <div>
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                <a href="#efficiency" className="inline-flex items-center gap-2 rounded-full bg-white px-5 py-3 text-sm font-semibold text-[#0A2463] transition hover:-translate-y-0.5">
                  Explore strategies <ArrowRight className="h-4 w-4" />
                </a>
                <a href="#roi" className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-5 py-3 text-sm font-semibold text-white transition hover:bg-white/15">
                  View ROI impact
                </a>
              </div>
            ...
            {workflowSteps.map((step, i) => {
              ...
              return (
                <button
                  ...
                  className={`text-left rounded-[28px] border p-6 shadow-sm transition hover:-translate-y-1 ${
                    active
                      ? "border-[#0A2463]/20 bg-[#0A2463] text-white shadow-lg shadow-blue-900/20"
                      : "border-slate-200 bg-white hover:shadow-md"
                  }`}
                >
                  <div className={`inline-flex h-12 w-12 items-center justify-center rounded-2xl ${active ? "bg-white/20" : "bg-[#0A2463]"} text-white`}>
                    <Icon className="h-5 w-5" />
                  </div>
                  <div className={`mt-2 text-[11px] font-semibold uppercase tracking-[0.16em] ${active ? "text-blue-100/70" : "text-slate-400"}`}>
                    Phase {step.step}
                  </div>
                  <div className={`mt-2 text-xl font-semibold ${active ? "text-white" : "text-slate-950"}`}>{step.title}</div>
                  {active && (
